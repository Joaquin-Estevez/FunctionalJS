...
uwOpponents = huskyGames2016.map( extractOpponent );
...
const huskiesLost = gameObj => {
    return gameObj.home === 'UW' ? gameObj.opponent_score > gameObj.home_score : gameObj.home_score > gameObj.opponent_score;
}

//console.log(huskiesLost(huskyGames2016[13]));
...
const gamesWeLost = huskyGames2016
  .filter( huskiesLost );

//console.log(gamesWeLost);

//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!

const namesOfGamesWeLost = huskyGames2016
  .filter( huskiesLost )
  .map( extractOpponent );

//console.log(namesOfGamesWeLost);
...
gamesWeLost.forEach( (game) => {
  // Build a string representing the game
  const summary = `${game.opponent} at ${game.home}, ${game.opponent_score} to ${game.home_score}`;
  //console.log(summary);
})
...
const mostYardsPassing = (game1, game2) => {
  // if doesn't have passing yards
  return !game1.passing_yards || game2.passing_yards > game1.passing_yards
  ? game2
  : game1;

}
//console.log(mostYardsPassing(huskyGames2016[0], huskyGames2016[4]));
...
// ejemplo de reduce()
const allOpponents = huskyGames2016.reduce( (opponentList, thisGame) => {
  // Get UW's opponent
  const opponent = extractOpponent(thisGame);
